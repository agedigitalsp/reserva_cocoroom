...
function enqueue_my_scripts() {
    if(is_home() || is_page(42) || is_page(323) || is_page(327)){
        wp_enqueue_script('scripts_reservas', plugin_dir_url(__FILE__). '/js/scripts_reservas.js', array('jquery'), '3.0', true);
...
//OBTENER JUEGOS DE LA CIUDAD
    ...
    if (!$query->have_posts()){
        ...
        while ($query->have_posts()) : $query->the_post();
            ...
            $precio_producto = get_post_meta( get_the_ID(), '_regular_price', true);

            if($local["id"] && $local["nombre"] && $horas && $lista_precios && $telefono_contacto_local && ( ($precio_producto && $slug_ciudad !== "coco-room-sevilla") || (!$precio_producto && $slug_ciudad === "coco-room-sevilla")) ){
...
function enviar_email_por_cada_reserva($order_id) {
    ...
    foreach ($order->get_items() as $item_key => $item) {
        ...
        if ( is_array( $datos_reserva ) ) {
            ...
            $email_torrejon = $id_ciudad == 36 ? ",torrejon@cocoroom.es" : "";

            do_action('send_mails_adpnsy_cllbck', $customer_email, 'Reserva confirmada: '.$nombre_juego, 'reserva_realizada', $data, array('Bcc: reservas@cocoroom.es'.$email_torrejon));
...
function cargar_reservas_calendario_2_callback_BACK() {
    global $wpdb;
    ...
    $resultados = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "reservas WHERE $where_1 estado NOT IN ('cancelada') $where_3 AND fecha_reserva BETWEEN '$start_date' AND '$end_date'"); 
...
/**
 * Función Núcleo: Calcula la disponibilidad real cruzando juegos, cierres y reservas.
 * Puede ser llamada desde AJAX, desde validaciones de WooCommerce, etc.
 */
function calcular_disponibilidad_sistema($id_ciudad, $num_usuarios, $fecha_reserva, $id_juego_especifico = false) {
    global $wpdb;

    $resultado = [
        'status' => true,
        'id_juego_especifico' => $id_juego_especifico,
        'reserva_telefonica' => false,
        'juegos' => [],
        'message' => ''
    ];

    // 1. Validaciones de fecha y reserva telefónica
    ...
    if ($fecha_reserva === $hoy->format('Y-m-d') || $fecha_reserva === $mañana->format('Y-m-d')) {
        $resultado['reserva_telefonica'] = true;
    }

    // 2. Obtener Juegos que cumplen el aforo
    $args = [
        'post_type' => 'product',
        'posts_per_page' => -1,
        'tax_query' => [['taxonomy' => 'product_cat', 'field' => 'id', 'terms' => $id_ciudad]],
    ];

    if($num_usuarios > 0){
        $args ['meta_query'] = [
            'relation' => 'AND',
            ['key' => 'numero_minimo_jugadores', 'value' => $num_usuarios, 'compare' => '<=', 'type' => 'NUMERIC'],
            ['key' => 'numero_maximo_jugadores', 'value' => $num_usuarios, 'compare' => '>=', 'type' => 'NUMERIC']
        ];
    }

    $query = new WP_Query($args);
    if (!$query->have_posts()) {
        return ['status' => false, 'message' => 'No hay juegos para ese número de personas'];
    }

    // 3. Obtener Reservas y Cierres de una vez para comparar
    $reservas = $wpdb->get_results($wpdb->prepare(
        "SELECT id_salas, id_juego, hora_reserva FROM {$wpdb->prefix}reservas 
         WHERE fecha_reserva = %s AND id_ciudad = %s AND estado IN ('pendiente', 'reservada')", 
        $fecha_reserva, $id_ciudad
    ), ARRAY_A);

    // Sumar items del carrito de WooCommerce
    if (function_exists('WC') && WC()->cart) {
        foreach (WC()->cart->get_cart() as $item) {
            if (isset($item['fecha_reserva']) && $item['fecha_reserva'] == $fecha_reserva) {
                $reservas[] = [
                    'id_salas' => strval($item['id_salas'] ?? ''),
                    'id_juego' => strval($item['product_id']),
    ...
    $cierres_raw = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}cierres WHERE id_ciudad = %s", $id_ciudad));
    
    // 4. Procesar cada juego y sus horas
    while ($query->have_posts()) : $query->the_post();
        $id_juego = get_the_ID();
        $local = obtener_ubicacion_juego($id_juego, 1);
        $salas = obtener_ubicacion_juego($id_juego, 2); 
        
        $modo_combate = get_post_meta($id_juego, 'modo_combate', true);

        // Obtener límites de personas configurados en el juego
        $min_jugadores = (int) get_post_meta($id_juego, 'numero_minimo_jugadores', true);
        $max_jugadores = (int) get_post_meta($id_juego, 'numero_maximo_jugadores', true);
        
        // Multiplicar instancias si el juego es modo simple y tiene múltiples salas
        $instancias_juego = [];
        if ($salas && (!$modo_combate || is_null($modo_combate))) {
            foreach ($salas as $sala) {
                $instancias_juego[] = [
                    'salas' => [$sala],
                    'id_salas_array' => [strval($sala['id'])]
                ];
            }
        } else {
            // Modo combate o juego sin salas: agrupar todas en una única instancia
            $id_salas_array = array_map(function($s) { return strval($s['id']); }, $salas ?: []);
            $instancias_juego[] = [
                'salas' => $salas,
                'id_salas_array' => $id_salas_array
            ];
        }

        $horario_meta = get_post_meta($id_juego, 'horario_juego', true);
        $lineas_horario = explode("\n", str_replace("\r", "", trim($horario_meta)));

        foreach ($instancias_juego as $instancia) {
            $id_salas_juego = $instancia['id_salas_array'];
            $slots_calculados = [];

            foreach ($lineas_horario as $linea) {
                if (empty(trim($linea))) continue;
                $parts = explode(' - ', $linea);
                $hora = trim($parts[0]);
                $porcentaje = isset($parts[1]) ? trim($parts[1]) : "";
                
                $esta_bloqueado = false;

                // Check Cierres
                foreach ($cierres_raw as $cierre) {
                    if (es_hora_bloqueada($cierre, $fecha_reserva, $hora, $id_juego, $id_salas_juego, $local['id'])) {
                        $esta_bloqueado = true;
                        break;
                    }
                }
                ...
                if (!$esta_bloqueado) {
                    foreach ($reservas as $res) {
                        if ($res['hora_reserva'] == $hora || explode(" - ", $res['hora_reserva'])[0] == $hora) {
                            $id_salas_res = array_filter(explode(',', $res['id_salas']));
                            if (quedan_salas_conflictivas($id_salas_juego, $id_salas_res, $res['id_juego'], $id_juego)) {
                                $esta_bloqueado = true;
                                break;
                            }
                        }
            ...
            $resultado['juegos'][] = [
                ...
                'color' => get_post_meta($id_juego, '_juego_tipo_', true) ?: 'Normal',
                'slots' => $slots_calculados,
                'direccion' => get_term_meta($local["id"], "direccion", true),
                'telefono' => get_term_meta($local["id"], 'telefono_contacto', true),
                'id_local' => $local["id"],
                'id_salas' => implode(',', $id_salas_juego),
                'datos_salas' => $instancia['salas']
            ];
        }
    endwhile;
    wp_reset_query();
...
/**
 * Helpers para la lógica de colisión
 */
function es_hora_bloqueada($cierre, $fecha_reserva, $hora, $id_juego, $id_salas_juego, $id_local) {
    $f_data = json_decode($cierre->fecha);
    $h_data = json_decode($cierre->hora);
    $t_data = json_decode($cierre->tipo_cierre);

    // 1. ¿Aplica a esta fecha?
    $aplica_fecha = false;
    if ($f_data->tipo == "fecha" && $f_data->valores == $fecha_reserva) {
        $aplica_fecha = true;
    } elseif ($f_data->tipo == "intervalo") {
        $v = explode(",", $f_data->valores);
        if ($fecha_reserva >= $v[0] && $fecha_reserva <= $v[1]) $aplica_fecha = true;
    } elseif ($f_data->tipo == "dias_semana") {
        $fecha_obj = new DateTime($fecha_reserva);
        $dias_map = [0 => 'domingo', 1 => 'lunes', 2 => 'martes', 3 => 'miercoles', 4 => 'jueves', 5 => 'viernes', 6 => 'sabado'];
        $dia_semana = $dias_map[$fecha_obj->format('w')];
        $v = explode(",", $f_data->valores);
        if (in_array($dia_semana, $v)) $aplica_fecha = true;
    }

    if (!$aplica_fecha) return false;

    // 2. ¿Aplica a esta hora?
    $hora_comparar = normalizar_hora($hora); // Normalizar 9:00 a 09:00
    $aplica_hora = false;
    if ($h_data->tipo == "todo_el_dia") $aplica_hora = true;
    elseif ($h_data->tipo == "hora" && $h_data->valores == $hora_comparar) $aplica_hora = true;
    elseif ($h_data->tipo == "intervalo") {
        $v = explode(",", $h_data->valores);
        if ($hora_comparar >= $v[0] && $hora_comparar <= $v[1]) $aplica_hora = true;
    }

    if (!$aplica_hora) return false;

    if ($t_data->tipo == "local" && $t_data->valores == $id_local) return true;
    if ($t_data->tipo == "juego") {
        $v = json_decode($t_data->valores);
        $salas_cierre = array_filter(explode(',', $v->id_salas));
        return quedan_salas_conflictivas($id_salas_juego, $salas_cierre, $v->id_juego, $id_juego);
    }
...
function normalizar_hora($hora) {
    ...
    if (strlen($hora_limpia) === 4) {
        return "0" . $hora_limpia;
    }
    return $hora_limpia;
}

function quedan_salas_conflictivas($salas_juego, $salas_bloqueo, $id_juego_bloqueo, $id_juego_actual) {
    if (!empty(array_intersect($salas_juego, $salas_bloqueo))) return true;
    if (empty($salas_juego) && empty($salas_bloqueo) && $id_juego_bloqueo == $id_juego_actual) return true;
    return false;
}

/**
 * Calcula la disponibilidad de un juego específico devolviendo todas sus salas/instancias.
 */
function calcular_disponibilidad_juego($id_ciudad, $num_usuarios, $fecha_reserva, $id_juego) {
    $disponibilidad = calcular_disponibilidad_sistema($id_ciudad, $num_usuarios, $fecha_reserva);

    if (!$disponibilidad['status']) {
        return $disponibilidad;
    }

    $juegos_filtrados = [];

    foreach ($disponibilidad['juegos'] as $juego) {
        if ($juego['id'] == $id_juego) {
            $juegos_filtrados[] = $juego;
        }
    }

    if (empty($juegos_filtrados)) {
        $disponibilidad['status'] = false;
        $disponibilidad['juegos'] = [];
        $disponibilidad['message'] = 'El juego solicitado no está disponible para el número de jugadores indicado o no existe.';
    } else {
        $disponibilidad['status'] = true;
        $disponibilidad['juegos'] = $juegos_filtrados;
        $disponibilidad['message'] = 'Disponibilidad del juego obtenida correctamente.';
    }
...
/**
 * Calcula la disponibilidad de un juego para una hora concreta.
 * Filtra los slots manteniendo únicamente el que coincide con la hora solicitada.
 */
function calcular_disponibilidad_juego_hora($id_ciudad, $num_usuarios, $fecha_reserva, $id_juego, $hora_reserva){
    $disponibilidad = calcular_disponibilidad_sistema($id_ciudad, $num_usuarios, $fecha_reserva);
    ...
    $hora_reserva = normalizar_hora($hora_reserva);

    foreach ($disponibilidad['juegos'] as $juego) {
        if ($juego['id'] == $id_juego) {
            $slots_filtrados = [];
            
            foreach ($juego['slots'] as $slot) {
                if (normalizar_hora($slot['hora']) === trim($hora_reserva) && $slot['disponible']) {
                    $slots_filtrados[] = $slot;
                    $hora_encontrada = true;
                }
            }

            if (!empty($slots_filtrados)) {
                $instancia_juego = $juego;
                $instancia_juego['slots'] = $slots_filtrados;
                $juegos_filtrados[] = $instancia_juego;
            }
        }
    }
    ...
    if (empty($juegos_filtrados)) {
        $disponibilidad['status'] = false;
        $disponibilidad['juegos'] = [];
        $disponibilidad['message'] = 'La hora solicitada no existe en el horario de este juego.';
    } else {
        $disponibilidad['juegos'] = $juegos_filtrados;
        $disponibilidad['status'] = $hora_encontrada; 
        $disponibilidad['message'] = $hora_encontrada ? 'La hora está disponible.' : 'La hora está bloqueada o reservada.';
    }
...
function add_rewrite_rules() {
    add_rewrite_rule('^coco-api/crear-reserva-pendiente/?$', 'index.php?coco_api_crear_reserva_pendiente_trigger=1', 'top');
    
    add_rewrite_rule('^coco-api/cancelar-reserva/?$', 'index.php?coco_api_cancelar_reserva_trigger=1', 'top');
}

add_filter('query_vars', 'add_query_vars_api_crear_reserva_pendiente');

function add_query_vars_api_crear_reserva_pendiente($vars) {
    $vars[] = 'coco_api_crear_reserva_pendiente_trigger';
    $vars[] = 'coco_api_cancelar_reserva_trigger';
    return $vars;
}



// ENDPOINT: CREAR RESERVA PENDIENTE
add_action('template_redirect', 'api_crear_reserva_pendiente');
function api_crear_reserva_pendiente() {
    if (get_query_var('coco_api_crear_reserva_pendiente_trigger') != 1) return;

    global $wpdb;
    $params = $_GET;

    if( !( isset($_GET["pass"]) && $_GET["pass"] === "F25K3XgoAAiRlxYD4cMT4teylQISAU46Za5klSkxxp5lj5ELWG" ) ){
        coco_log("Contraseña incorrecta en función api_crear_reserva_pendiente.");
        output_error_frc('No se ha podido generar la reserva debido a que el token de acceso es incorrecto. Hay un problema técnico y lo solucionaremos lo antes posible. Disculpe las molestias.');
        exit;
    }

    // Recoger parámetros
    $id_ciudad = intval($params['id_ciudad']);
    $id_juego = intval($params['id_juego']);
    $fecha_reserva = sanitize_text_field($params['fecha_reserva']);
    $hora_reserva = sanitize_text_field($params['hora_reserva']);
    $num_usuarios = intval($params['num_usuarios']);
    $nombre = isset($params['nombre']) ? sanitize_text_field($params['nombre']) : "";
    $apellidos = isset($params['apellidos']) ? sanitize_text_field($params['apellidos']) : "";
    $email = isset($params['email']) ? sanitize_email($params['email']) : "";
    $telefono = isset($params['telefono']) ? sanitize_text_field($params['telefono']) : "";

    // Comprobar si ya existe una reserva activa para esa fecha/hora
    $disponibilidad_juego_hora = calcular_disponibilidad_juego_hora($id_ciudad, $num_usuarios, $fecha_reserva, $id_juego, $hora_reserva);

    if (!$disponibilidad_juego_hora["status"]) output_error_frc('No hay disponibilidad para los datos solicitados');
    if (empty($disponibilidad_juego_hora["juegos"])) output_error_frc('No hay datos de juegos disponibles');
    if ($disponibilidad_juego_hora["reserva_telefonica"]) output_error_frc('No se puede hacer la reserva porque es una reserva de última hora. Es necesario contactar vía telefónica');

    $juego_disponible = reset($disponibilidad_juego_hora["juegos"]);

    $skey = funcion_generar_key_reserva(); 
    $nombre_juego = get_the_title($id_juego);
    $nombre_ciudad = get_term_by('id', $id_ciudad, 'product_cat')->name;
    $hora_reserva = preg_replace('/^0(?=[1-9]:)/', '', trim($hora_reserva)); //LIMPIAR POSIBLE CERO a la izquierda

    $id_local = intval($juego_disponible['id_local']);
    $id_salas = sanitize_text_field($juego_disponible['id_salas']);

    $wpdb->insert(
        $wpdb->prefix . 'reservas',
        array(
            'id_order' => 0, // No hay pedido todavía
            'fecha_hora_creacion_reserva' => date("Y-m-d H:i:s"),
            'id_ciudad' => $id_ciudad,
            'id_local' => $id_local,
            'id_salas' => $id_salas,
            'id_juego' => $id_juego,
            ...
            'num_usuarios' => $num_usuarios,
            'jugadores' => json_encode([]),
            'fecha_reserva' => $fecha_reserva,
            'hora_reserva' => $hora_reserva,
            'skey' => $skey,
            'estado' => 'pendiente'
        ),
        array('%d','%s','%d','%d','%s','%d','%s','%s','%d','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')
    );

    $id_reserva = $wpdb->insert_id;

    // Programar el CRON para cancelar la reserva en 1 hora si no se paga
    wp_schedule_single_event(time() + 3600, 'cocoroom_cancelar_reserva_cron', array($id_reserva));

    // Enlace mágico
    $url_pago = add_query_arg(array(
        'id' => $id_reserva,
        'key'       => $skey
    ), wc_get_checkout_url());
    ...
    //NOTIFICAR - reserva_enlace_pago
    coco_log("El enlace de pago generado correctamente.");
    if(!$email){
        coco_log("Notificación reserva_enlace_pago NO ENVIADA POR EMAIL: email no definido");
    }
    else{
        coco_log("Notificando reserva_enlace_pago...");
        ...
        do_action('send_mails_adpnsy_cllbck', $email, 'Enlace de pago de tu reserva', 'reserva_enlace_pago', $data);
    }

    output_response_frc([
        'status' => true,
        'enlace_pago' => $url_pago,
        'notificado_email' => $notificado_email,
...
// ENDPOINT: CANCELAR RESERVA POR EMAIL, DÍA Y HORA
add_action('template_redirect', 'api_cancelar_reserva');
function api_cancelar_reserva() {
    if (get_query_var('coco_api_cancelar_reserva_trigger') != 1) return;

    global $wpdb;
    $params = $_GET;

    if( !( isset($_GET["pass"]) && $_GET["pass"] === "lE5WEpqXfw5AJdXiAw7CiVYFh1wbtmv00B78yuXIaD5ZusR9ZC" ) ){
        coco_log("Contraseña incorrecta en función api_cancelar_reserva.");
        output_error_frc('Token de acceso incorrecto.', 401);
        exit;
    }

    $email = isset($params['email']) ? sanitize_email($params['email']) : "";
    $fecha_reserva = isset($params['fecha_reserva']) ? sanitize_text_field($params['fecha_reserva']) : "";
    $hora_reserva = isset($params['hora_reserva']) ? sanitize_text_field($params['hora_reserva']) : "";

    if (empty($email) || empty($fecha_reserva) || empty($hora_reserva)) {
        output_error_frc('Faltan parámetros obligatorios (email, fecha_reserva y hora_reserva).');
        exit;
    }

    $hora_reserva = preg_replace('/^0(?=[1-9]:)/', '', trim($hora_reserva));

    $reserva = $wpdb->get_row($wpdb->prepare(
        "SELECT * FROM {$wpdb->prefix}reservas WHERE email_usuario = %s AND fecha_reserva = %s AND hora_reserva = %s",
        $email, $fecha_reserva, $hora_reserva
    ));

    if (!$reserva) {
        output_error_frc('No se ha encontrado ninguna reserva activa con los datos facilitados.', 404);
        exit;
    }

    if ($reserva->estado === "cancelada") {
        output_error_frc('La reserva ya está cancelada.', 404);
        exit;
    }
    if ($reserva->estado !== "pendiente" && $reserva->estado !== "reservada" ) {
        output_error_frc('No se puede cancelar la reserva.', 404);
        exit;
    }
    ...
    // NOTIFICAR reserva_cancelada_expirada
    coco_log("Reserva ID {$reserva->id_reserva} encontrada. Notificando reserva_cancelada_expirada.");
    ...
    $data = array(
        ...
        "Direccion_local" => $direccion_local,
        "Enlace_local" => $enlace_local,
        "Enlace_cancelar_reserva" => home_url("/") . "cancelar-reserva/?id=" . $reserva->id_reserva . "&key=" . $reserva->skey,
        "Telefono_contacto_franquicia" => $telefono_contacto_franquicia,
        "Email_contacto_franquicia" => $email_contacto_franquicia,
    );
    coco_log($email_usuario);
    coco_log(print_r($data,true));

    do_action('send_mails_adpnsy_cllbck', $email_usuario, '¿Quieres cancelar tu reserva?', 'solicitud_cancelar_reserva', $data);

    output_response_frc([
        'status' => true,
        'message' => 'Email con enlace de cancelación de la reserva enviado correctamente. Es necesario acceder al email para confirmar que es la reserva es del cliente.'
    ]);
    exit;
}
...
// CRON PARA CANCELAR LA RESERVA TEMPORAL SI NO SE COMPLETA
add_action('cocoroom_cancelar_reserva_cron', 'ejecutar_cancelacion_reserva_cron');
function ejecutar_cancelacion_reserva_cron($id_reserva) {
    global $wpdb;
    $reserva = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}reservas WHERE id_reserva = %d AND id_order = 0 AND estado LIKE 'pendiente'", $id_reserva));

    if($reserva){
        coco_log("El enlace de pago va a ser cancelado (placeholder de reserva eliminada)");
        coco_log(print_r($reserva,true));

        $cancelada = $wpdb->query($wpdb->prepare(
            "DELETE FROM {$wpdb->prefix}reservas WHERE id_reserva = %d AND id_order = 0 AND estado LIKE 'pendiente'", 
            $id_reserva
        ));

        coco_log("Reserva cancelada: ". $cancelada);
        // NOTIFICAR reserva_cancelada_expirada
        if(!$reserva->email_usuario){
            coco_log("Notificación reserva_cancelada_expirada NO ENVIADA POR EMAIL: email no definido");
        }
        else{
            coco_log("Notificando reserva_cancelada_expirada...");
            ...
            coco_log($reserva->email_usuario);
            coco_log(print_r($data,true));

            do_action('send_mails_adpnsy_cllbck', $reserva->email_usuario, 'Reserva cancelada', 'reserva_cancelada_expirada', $data);
        }
    }
}
...
// INTERCEPTAR EL ENLACE DE PAGO, LIMPIAR CARRITO Y AÑADIR PRODUCTO
add_action('template_redirect', 'procesar_enlace_completar_reserva');
function procesar_enlace_completar_reserva() {
    if ( function_exists('is_checkout') && is_checkout() && isset($_GET['id'], $_GET['key']) && function_exists('WC') ) {
        global $wpdb;
        $id_reserva = intval($_GET['id']);
        $skey = sanitize_text_field($_GET['key']);

        // Comprobar que la reserva existe y sigue pendiente
        $reserva = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}reservas WHERE id_reserva = %d AND skey = %s AND estado LIKE 'pendiente' AND id_order = 0", $id_reserva, $skey));
        ...
        if ($reserva) {
            ...
            $cart_item_data = array(
                ...
                'fecha_reserva' => $reserva->fecha_reserva,
                'num_usuarios' => $reserva->num_usuarios,
                'confirmar_id_reserva' => $reserva->id_reserva,
                'confirmar_reserva_skey' => $reserva->skey
            );

            WC()->cart->add_to_cart($reserva->id_juego, 1, 0, array(), $cart_item_data);

            WC()->session->set('datos_usuario_reserva_temp', array(
                'billing_first_name' => $reserva->nombre_usuario,
                'billing_last_name' => $reserva->apellidos_usuario,
                'billing_email' => $reserva->email_usuario,
                'billing_phone' => $reserva->telefono_usuario
            ));

            WC()->session->set('aviso_reserva_pendiente', true);
        } else {
            WC()->cart->empty_cart();
            wp_die('Lo sentimos, este enlace de pago no es válido o ha expirado.', "Error - COCO ROOM");
        }
    }
}
...
add_action('woocommerce_before_checkout_form', 'mostrar_mensaje_checkout_reserva_pendiente', 10);
function mostrar_mensaje_checkout_reserva_pendiente() {
    if (WC()->session && WC()->session->get('aviso_reserva_pendiente')) {
        wc_print_notice('Estás completando una reserva pendiente. Por favor, finaliza el pedido para confirmar tu reserva.', 'notice', array('info' => 'info_reserva_pendiente'));
        WC()->session->__unset('aviso_reserva_pendiente');
    }
}



// AUTOCOMPLETAR LOS CAMPOS DEL CHECKOUT CON LA SESIÓN
add_filter('woocommerce_checkout_get_value', 'autocompletar_checkout_reserva_temporal', 10, 2);
function autocompletar_checkout_reserva_temporal($value, $input) {
    if (isset(WC()->session)) {
        $datos = WC()->session->get('datos_usuario_reserva_temp');
        if (!empty($datos) && isset($datos[$input])) {
            return $datos[$input];
        }
    }
    return $value;
...
// AÑADIR LAS VARIABLES DE CONTROL A LOS METADATOS DEL PEDIDO
// Usamos prioridad 11 para ejecutarnos justo después de tu función 'guardar_meta_en_pedido'
add_action('woocommerce_checkout_create_order_line_item', 'guardar_meta_control_en_pedido', 11, 4);
function guardar_meta_control_en_pedido($item, $cart_item_key, $values, $order) {
    $meta_data = $item->get_meta('datos_reserva', true);
    if (!is_array($meta_data)) $meta_data = array();

    if (isset($values['confirmar_id_reserva'])) {
        $meta_data['confirmar_id_reserva'] = $values['confirmar_id_reserva'];
        $meta_data['confirmar_reserva_skey'] = $values['confirmar_reserva_skey'];
        $item->update_meta_data('datos_reserva', $meta_data);
    }
}



// LIMINAR LA RESERVA TEMPORAL JUSTO ANTES DE QUE SE CREE LA DEFINITIVA
add_action('woocommerce_checkout_order_processed', 'machacar_reserva_temporal_pre_procesado', 5, 1);
function machacar_reserva_temporal_pre_procesado($order_id) {
    global $wpdb;
    $order = wc_get_order($order_id);
    $items = $order->get_items();
    $first_item = reset($items);
    ...
    $datos_reserva = $first_item->get_meta('datos_reserva', true);

    if (isset($datos_reserva['confirmar_id_reserva']) && isset($datos_reserva['confirmar_reserva_skey'])) {
        $wpdb->delete(
            $wpdb->prefix . 'reservas',
            array(
                'id_reserva' => intval($datos_reserva['confirmar_id_reserva']),
                'skey' => sanitize_text_field($datos_reserva['confirmar_reserva_skey']),
                'id_order' => 0 
            ),
            array('%d', '%s', '%d')
        );

        WC()->session->__unset('datos_usuario_reserva_temp');
    }
}



// HELPER RESPUESTAS JSON
function output_response_frc($data, $code = 200) {
    status_header($code);
    nocache_headers();
...
function output_error_frc($message, $code = 400) {
    output_response_frc([
        'status' => false,
        'message' => $message
    ], $code);
