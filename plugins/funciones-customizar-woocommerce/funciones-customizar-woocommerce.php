...
function custom_woocommerce_category_fields($taxonomy) {
    ...
    <div class="campos_ciudad" style="display: none;">
        ...
        <div class="form-field">
            <label for="activar_faqs"><?php echo 'Activar preguntas frecuentes en esta ciudad'; ?></label>
            <input type="checkbox" name="activar_faqs" id="activar_faqs">
                Habilitar para mostrar el panel de FAQs en los perfiles de Franquicia.
        </div>

        <div class="form-field">
            <label for="activar_estadisticas"><?php echo 'Activar Estadísticas en esta ciudad'; ?></label>
            <input type="checkbox" name="activar_estadisticas" id="activar_estadisticas">
                Habilitar para mostrar el panel de Estadísticas en los perfiles de Franquicia.
        </div>

        <div class="form-field">
            <label for="enlace_clickup_tickets"><?php echo 'Enlace a tickets de ClickUp para incidencias durante las partidas (puede estar vacío)'; ?></label>
            <input type="text" name="enlace_clickup_tickets" id="enlace_clickup_tickets" placeholder="">
        </div>

        <div class="form-field">
            <label for="bloquear_partidas_gamemasters"><?php echo 'Bloquear reservas antiguas para Game Masters'; ?></label>
            <input type="checkbox" name="bloquear_partidas_gamemasters" id="bloquear_partidas_gamemasters">
                Deshabilita la opción de editar, empezar o cerrar sesiones del mes anterior.
        </div>

        <div class="form-field">
            <label for="cron_email_gamemasters"><?php echo 'Notificar recurrentemente a los Game Masters'; ?></label>
            <input type="checkbox" name="cron_email_gamemasters" id="cron_email_gamemasters">
                Los Game Masters con partidas pendientes de cerrar recibirán un email recordatorio cada día.
        </div>
    </div>
...
function save_custom_woocommerce_category_fields($term_id) {
    ...
    if (isset($_POST['enlace_clickup_tickets'])) {
        update_term_meta($term_id, 'enlace_clickup_tickets', sanitize_text_field($_POST['enlace_clickup_tickets']));
    }

    if (isset($_POST['activar_faqs'])) {
        update_term_meta($term_id, 'activar_faqs', 1);
    }
    else{
        update_term_meta($term_id, 'activar_faqs', "");
    }

    if (isset($_POST['activar_estadisticas'])) {
        update_term_meta($term_id, 'activar_estadisticas', 1);
    }
    else{
        update_term_meta($term_id, 'activar_estadisticas', "");
    }


    if (isset($_POST['bloquear_partidas_gamemasters'])) {
        update_term_meta($term_id, 'bloquear_partidas_gamemasters', 1);
    }
    else{
        update_term_meta($term_id, 'bloquear_partidas_gamemasters', "");
    }


    if (isset($_POST['cron_email_gamemasters'])) {
        update_term_meta($term_id, 'cron_email_gamemasters', 1);
    }
    else{
        update_term_meta($term_id, 'cron_email_gamemasters', "");
    }
...
function edit_custom_woocommerce_category_fields($term, $taxonomy) {
    ...
    $enlace_clickup_tickets = get_term_meta($term->term_id, 'enlace_clickup_tickets', true);

    $activar_faqs = get_term_meta($term->term_id, 'activar_faqs', true);
    $activar_estadisticas = get_term_meta($term->term_id, 'activar_estadisticas', true);
    
    $bloquear_partidas_gamemasters = get_term_meta($term->term_id, 'bloquear_partidas_gamemasters', true);
    $cron_email_gamemasters = get_term_meta($term->term_id, 'cron_email_gamemasters', true);
    ...
    ?>
        ...
        <tr class="form-field">
            <th scope="row"><label for="enlace_clickup_tickets"><?php echo 'Enlace a tickets de ClickUp para incidencias durante las partidas (puede estar vacío)'; ?></label></th>
            <td>
                <input type="text"  name="enlace_clickup_tickets" id="enlace_clickup_tickets" placeholder="" value="<?php echo esc_attr($enlace_clickup_tickets); ?>">
            </td>
        </tr>

        <tr class="form-field">
            <th scope="row"><label for="activar_faqs"><?php echo 'Activar preguntas frecuentes en esta ciudad'; ?></label></label></th>
            <td>
                <input type="checkbox" name="activar_faqs" id="activar_faqs" value="on" <?php echo $activar_faqs != '' ? "checked" : "" ?>>
                Habilitar para mostrar el panel de FAQs en los perfiles de Franquicia.
            </td>
        </tr>

        <tr class="form-field">
            <th scope="row"><label for="activar_estadisticas"><?php echo 'Activar Estadísticas en esta ciudad'; ?></label></label></th>
            <td>
                <input type="checkbox" name="activar_estadisticas" id="activar_estadisticas" value="on" <?php echo $activar_estadisticas != '' ? "checked" : "" ?>>
                Habilitar para mostrar el panel de Estadísticas en los perfiles de Franquicia.
            </td>
        </tr>

        <tr class="form-field">
            <th scope="row"><label for="bloquear_partidas_gamemasters"><?php echo 'Bloquear reservas antiguas para Game Masters'; ?></label></label></th>
            <td>
                <input type="checkbox" name="bloquear_partidas_gamemasters" id="bloquear_partidas_gamemasters" value="on" <?php echo $bloquear_partidas_gamemasters != '' ? "checked" : "" ?>>
                Deshabilita la opción de editar, empezar o cerrar sesiones del mes anterior.
            </td>
        </tr>

        <tr class="form-field">
            <th scope="row"><label for="cron_email_gamemasters"><?php echo 'Notificar recurrentemente a los Game Masters'; ?></label></label></th>
            <td>
                <input type="checkbox" name="cron_email_gamemasters" id="cron_email_gamemasters" value="on" <?php echo $cron_email_gamemasters != '' ? "checked" : "" ?>>
                Los Game Masters con partidas pendientes de cerrar recibirán un email recordatorio cada día.
            </td>
        </tr>
